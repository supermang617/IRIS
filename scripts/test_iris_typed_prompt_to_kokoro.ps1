[CmdletBinding()]
param(
    [string] $Prompt = "Hello Iris, respond in one short sentence.",
    [switch] $NoPlay
)
...
Write-Section "Run typed prompt through Iris local response path"

$responseOutput = Invoke-Captured "Iris typed prompt response" "cargo" @(
    "run",
    "-p",
    "iris-runtime",
    "--",
...
if ($responseOutput -notmatch "Response post-check:\s+PASS") {
    throw "Iris response post-check did not pass."
}

$match = [regex]::Match($responseOutput, "HUD response:\s*(?<response>[\s\S]*?)\r?\nResult:\s*PASS")

if (-not $match.Success) {
    throw "Could not extract HUD response from Iris output."
}
...
if ([string]::IsNullOrWhiteSpace($reply)) {
    throw "Iris produced an empty reply."
}

if ($reply.Length -gt 300) {
    throw "Iris reply is too long for this milestone: $($reply.Length) characters."
}
...
$speakArgs = @(
    ...
    "-Text", $reply,
    "-OutWav", ".iris-dev\diagnostics\typed-to-kokoro-response.wav",
    "-PlaybackSeconds", "6"
)
...
if ($LASTEXITCODE -ne 0) {
    throw "Kokoro speech failed."
}

Write-Section "Result"
Write-Host "PASS: typed prompt -> Iris/Qwen response -> Kokoro speech completed."
